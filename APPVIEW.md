...
- **No identity resolution beyond DIDs.** It does not join references to
  ...
  by descriptor and external identifiers, but a person's name only
  publishes when it already carries a DID or a public-authority id
  (wikidata, viaf, isni, orcid, musicbrainz, discogs, ipi), so people are
  never matched by name or by contact details. The index does not group
  refs yet.
