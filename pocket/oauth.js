/* ATProto OAuth for the Pocket Totem — a public browser client.
 *
...
function idb(mode, fn) {
  return new Promise((res, rej) => {
    const open = indexedDB.open('pt-oauth', 1);
...
export async function startLogin(handle) {
  ...
  sessionStorage.setItem('pt:oauth-pending', JSON.stringify(
...
export async function completeLogin() {
  ...
  const pending = JSON.parse(sessionStorage.getItem('pt:oauth-pending') || 'null');
  sessionStorage.removeItem('pt:oauth-pending');
  ...
  localStorage.setItem('pt:session', JSON.stringify(session));
  return session;
}

export function session() {
  try { return JSON.parse(localStorage.getItem('pt:session') || 'null'); }
  catch (e) { return null; }
}
...
async function fresh() {
  ...
  const next = { ...s, access: tok.access_token,
    ...
    expires: Date.now() + (tok.expires_in || 3600) * 1000 };
  localStorage.setItem('pt:session', JSON.stringify(next));
  return next;
}
